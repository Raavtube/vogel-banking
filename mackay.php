...
for ($count = 0; $count < $rows_count; $count++) {
  ...
  $sql = "SELECT balance, reason from $tableName where id = $rowToFetch"; // SQL with parameters
  $stmt = $conn->prepare($sql); 
  $stmt->bind_param("i", $redirectID);
  $stmt->execute();
  $result = $stmt->get_result(); // get the mysqli result
...
<html>
<head>
<title>Mackay Vogel Banking</title>
<style>
#table {
...
<center><h1>Mackay Vogel Banking</h1></center>

<center>
<h2>Current Balance: $<?php echo $currentBal; ?> | Paypal Balance: $<?php echo $paypalBal; ?></h2>
<form action="m_getResult.php" method="post">
